...
在字面量字符串中也可以用十六进制或八进制转义字符来包含任意字节。十六进制的转义字符写作 \xhh，h为十六进制数且必须为两位；八进制转义字符写作 \ooo，不超过 \377。这两者都表示单个字节。
...
```go
"世界"
"\xe4\xb8\x96\xe7\x95\x8c" \\ unicode编码，同时也是在内存中的数值
"\u4e16\u754c" \\ 2字节UTF8编码
"\U00004e16\U0000754c" \\ 4字节UTF8编码
```

码点小于256的 rune 可以用单个十六进制转义符来表示，如 'A' 写作 '\x41'，但对于更大的值，必须使用 \u 或者 \U 转义符。因此，'\xe4\xb8\x96' 不是一个合法的 rune 字面量。
...
当 UTF-8 编码字符串用 \[]rune 转换时，返回字符串对应的 unicode 码点序列：
...
```go
fmt.Println(ZiB / (1 << 7)) // constant 9223372036854775808 overflows int
fmt.Println(ZiB/(1<<7) - 1) // 9223372036854775807
```

💡 默认表示类型为 int，所以可能以 int 为参照判断一出。
